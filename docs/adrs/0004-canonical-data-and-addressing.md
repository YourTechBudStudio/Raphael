...
Use Turso Database, the Rust SQLite rewrite rather than libSQL, with Drizzle for persistence. Immutable entity IDs, stable parent references, and mutable slugs define the hierarchy. Slugs share one unique namespace across all sibling entity types, including archived entities. Core rejects collisions rather than silently choosing another slug; title changes do not implicitly change slugs.
...
- The chosen Turso release and Drizzle integration must be verified together for recursive queries, JSON filtering, transactions, and full-text search; SQLite compatibility is not proof of feature parity.
