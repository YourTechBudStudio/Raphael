...
/**
 * A capture owner against a real server and a real local database.
 *
 * Only the fetch and the editor are substituted, and the editor only because there is no WebView
 * here. Everything else - the frozen bytes, the intent transaction, the sweep on reopen, the replay
 * under the original key - is the code the app runs.
 */
