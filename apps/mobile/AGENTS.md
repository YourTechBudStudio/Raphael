...
Raphael's Expo React Native app for Android and iOS. Areas, projects, and notes are read from and created on the connected Raphael server through `@raphael/client`. A note is written into a local SQLite draft before a composer opens over it, and its Save is a durable attempt, so unfinished writing and an unanswered save both survive the process. Creating an area or a project is a plain request from the sheet that asked for it, with no durable record. Favorites and active-project selections, and recorded media, have no server operation yet and are kept in memory for the session, scoped to the connection; nothing in the app may present those as saved.
...
Current modules: `connection` owns the server connection, its secure storage, and the transition between one connection and the next; `home` composes the feed and active projects; `collections` owns the hierarchy query, area/project screens, container creation, favorites, and active selection; `browse` owns tree browsing and publishes the hierarchy tree, including the selectable variant capture's destination picker draws; `search` owns scoped search; `capture` owns the durable note owner, the composer, the destination picker, the unfinished-note projection, and the recovery screen, as well as voice capture; `editor` owns the bundled TipTap renderer and its bridge; `resources` owns server note retrieval and presentation; `playback` owns shared playback; `navigation` owns route helpers, navigation chrome, and the mutually exclusive sheet state.
...
- Keep cache maintenance behind capability interfaces. UI callers should not coordinate other modules' private cache details. Use the app-wide QueryClient rather than creating isolated caches per module. A successful note write seeds and invalidates through the resources module; a successful container write is handled inside collections, which seeds and invalidates together and fences both by the activation the write was made under.
- Notes have one dispatcher, app-scoped, in `modules/capture`, mounted above the connection gate so an answer arriving after a switch still has somewhere to be written. Nothing outside that capability may open its database or re-derive a standing from its records. Persist the draft before opening a composer over it, persist the attempt before sending it, persist the answer before presenting it, and clear a record only once it has been consumed and both memory and the database agree nothing newer exists.
- Creating an area or a project is one request from the form that asked for it: a captured transport and activation, one idempotency key per opening reused across presses, and one request in flight. Closing forgets the key. There is no durable container attempt, no container recovery, and `raphael-creation.db` is never opened again - and never deleted, because this codebase does not remove a local database on its own.
- Setup is a prerequisite for using the app. Without a configured connection the setup screen is the whole app: no Home, no composer, and no reading, copying or discarding of anything on this phone. Work belonging to a retired connection becomes reachable afterwards, in recovery, offering only an explicit copy into a new note here or a discard. Nothing is ever rebound to a new connection by matching an endpoint.
...
- A success is reported when it is shown, and its receipt is spent then - never on mount, on navigation, or on a cache refresh. A receipt that outlives its process is reported on the next launch rather than discarded.
